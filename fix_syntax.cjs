...
files.forEach(f => {
    ...
    // We need to clean up the bad code.
    // It looks like: previewBox.innerText = 'Total: (e, id) { ... 
    // And it repeats 'function addToCart' at the end of the giant copy.
    // Let's just find the first calculateItemTotal and cut out that entire script block, 
    // then restore the proper one.
    
    // Find the injected script starting from calculateItemTotal
    // Since we know the previous good function was `updateCartCount` or something.
    // Let's use a regex to replace everything from `function calculateItemTotal` to `function addToCart(e, id)`
    const fixRegex = /function calculateItemTotal[\s\S]*?function addToCart\(e, id\)/;
    
    const correctCode = `function calculateItemTotal(productBasePrice, pricingUnit, amount, reqUnit) {
            let cost = 0;
            if (pricingUnit === 'kg') {
    ...
    // We can just use split/join instead of replace to avoid $ expansion
    content = content.split(fixRegex).join(correctCode);
    
    // Now we also need to fix the template
    // The previous regex didn't match. 
    // Let's replace the whole `grid.innerHTML = prods.map...` to `.join('');` chunk.
    const mapRegex = /grid\.innerHTML = [a-zA-Z]+\.map\([\s\S]*?\}\)\.join\(''\);/;
    
    const arrayName = f === 'index.html' ? 'prods' : 'catProds'; // index.html uses prods.map, others use catProds.map
    
    const newTemplate = `grid.innerHTML = ${arrayName}.map(p => {
    ...
    content = content.split(mapRegex).join(newTemplate);

    fs.writeFileSync(f, content, 'utf8');
});

// Also fix cart.html which got corrupted? Cart.html showed SyntaxError!
let cartContent = fs.readFileSync('cart.html', 'utf8');
let checkSyntax = cartContent.match(/function calcCardPreview/);
if (checkSyntax) {
   // Did we run step2 on cart.html? No, array only had index and categories.
}
// Let's check cart.html syntax anyway.
